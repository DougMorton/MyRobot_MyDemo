...
rtcMultiConnection.onFileEnd = function(file) {
	...
	addNewMessage({
		message: '<a href="' + file.url + '" download="' + file.name + '">Download <strong style="color:red;">' + file.name + '</strong> </a>'
	});
    if (file.type.indexOf('image') != -1) {
        //div.innerHTML = '<a href="' + file.url + '" download="' + file.name + '">Download <strong style="color:red;">' + file.name + '</strong> </a><br /><img src="' + file.url + '" title="' + file.name + '" style="max-width: 80%;">';
        div.innerHTML = '<img src="' + file.url + '" title="' + file.name + '" style="max-width: 100%;">';
      	$("#dialog-popup" ).dialog({
    		autoOpen: false,
    		width: 800
  		});
